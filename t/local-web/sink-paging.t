  ...
  return remote ($c, {
    a => [{'Content-Type' => 'text/xml'}, q{
      <rss>
        ...
        <link>https://url/</link>
        <description>This is Hoge Feed @@TIME@@</description>
        <item>
    ...
    return POST ($c, qq{/source/$source->{source_id}/enqueue}, {})->then (sub {
      ...
      test {
        ...
        is $res->header ('Content-Type'), 'application/json; charset=utf-8';
        is 0+@{$json->{items}}, 0;
        is $json->{next_url}, $next;
      } $c;
    });
  })->then (sub { done $c; undef $c });
} wait => $wait, n => 10, name => 'next empty page';
...
test {
  ...
  return remote ($c, {
    ...
    my $next = $json->{next_url};
    return POST ($c, qq{/process/$process->{process_id}}, {
      process_options => perl2json_chars {
        ...
        steps => [
          ...
          {name => 'parse_rss'},
          {name => 'set_key', field => 'url'},
          {name => 'delete_substring', field => 'title', regexp => '^Feed '},
        ],
      ...
      test {
        ...
        is $item->{data}->{title}, 'Entry';
        ok $json->{next_url};
        isnt $json->{next_url}, $next;
      } $c;
    });
  })->then (sub { done $c; undef $c });
} wait => $wait, n => 12, name => 'next page, reprocessed item';
